First command we run is 'ls' to view all the files in the directory.
after running the command we can see inhere directory is present.
To access the inhere directory we have to first change the change the directory from home to inhere which can be done by running the 'cd' command which refers to change directory.
cd inhere/ 
